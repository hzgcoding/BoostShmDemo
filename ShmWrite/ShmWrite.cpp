// ShmWrite.cpp : Defines the entry point for the console application.
//
...
int main()
{
	...
	typedef basic_string<char, std::char_traits<char>, CharAllocator> BoostString;
	bool running = true;
	...
	while (running)
	{
		// write share memory
		std::cout << "----------" << std::endl;
		BoostString *s = segment.find_or_construct<BoostString>("Hello")("", segment.get_segment_manager());
		s->clear();
